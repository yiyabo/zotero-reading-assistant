...
async function main() {
  ...
  const outfile = path.join(buildDir, "addon/content/scripts/__addonRef__.js");
  await esbuild
    .build({
      ...
      outfile,
      banner: {
        js: 'var __dirname="",__filename="",process={env:{}},Buffer=undefined,module={exports:{}},exports=module.exports;',
      },
  ...
  // Rename the output file from __addonRef__.js to the actual addonRef
  const addonRef = config.addonRef;
  const renamedOutfile = path.join(buildDir, `addon/content/scripts/${addonRef}.js`);
  fs.renameSync(outfile, renamedOutfile);
  console.log(`[Build] Renamed ${outfile} -> ${renamedOutfile}`);
