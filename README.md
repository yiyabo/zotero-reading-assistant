...
Zotero 论文阅读助手：在 Zotero 侧边栏中直接和论文、PDF、笔记与知识图谱对话。
...
- **局部追问**：可以从任意 AI 回答旁打开独立追问窗口，围绕当前回答继续深入，不污染主对话。
...
- **独立知识 Wiki**：把图谱中的论文、方法、数据集和研究方向沉淀成可浏览的 Wiki 页面，并支持为每个页面追加个人 Markdown 备注。
...
- **深挖单个回答**：对某段解释、公式、实验结论或方法细节单独开追问线程。
...
- 打开 Zotero 的插件页面
- 选择“从文件安装插件”
- 选中 `builds/zotero-reading-assistant.xpi`

安装完成后，在插件设置页填写模型 API Key、Base URL 和 Model 即可开始使用。
